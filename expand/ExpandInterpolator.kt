...
class ExpandInterpolator : android.view.animation.Interpolator {

    private var mValues: FloatArray? = null
    private var mStepSize = 0f

    /**
     * Lookup table values sampled with x at regular intervals between 0 and 1 for a total of 201 points.
     */
    ...
    override fun getInterpolation(input: Float): Float {
        if (1.0f <= input) return 1.0f
        if (0f >= input) return 0f
        val position = Math.min((input * (mValues!!.size - 1)).toInt(), mValues!!.size - 2)
        return mValues!![position] + (input - position * mStepSize) / mStepSize * (mValues!![position + 1] - mValues!![position])
    }
}
